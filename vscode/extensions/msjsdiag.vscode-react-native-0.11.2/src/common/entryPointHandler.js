...
class EntryPointHandler {
    ...
    runFunction(taskName, error, codeToRun, errorsAreFatal = false, extProps) {
        return this.runFunctionWExtProps(taskName, extProps || {}, error, codeToRun, errorsAreFatal);
    }
    runFunctionWExtProps(taskName, extProps, error, codeToRun, errorsAreFatal = false) {
        return this.handleErrors(error, telemetryHelper_1.TelemetryHelper.generate(taskName, extProps, codeToRun), /*errorsAreFatal*/ errorsAreFatal);
    }
    // This method should wrap the entry point of the whole app, so we handle telemetry and error reporting properly
    runApp(appName, appVersion, error, reporter, codeToRun, extProps) {
        try {
