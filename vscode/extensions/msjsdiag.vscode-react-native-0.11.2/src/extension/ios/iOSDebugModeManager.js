...
class IOSDebugModeManager {
    constructor(projectRoot, scheme) {
        this.logger = OutputChannelLogger_1.OutputChannelLogger.getMainChannel();
        this.projectRoot = projectRoot;
        this.simulatorPlist = new simulatorPlist_1.SimulatorPlist(this.projectRoot, scheme);
    }
    setSimulatorRemoteDebuggingSetting(enable, configuration, productName) {
    ...
    getSimulatorRemoteDebuggingSetting(configuration, productName) {
        return this.findPListFile(configuration, productName)
            .then((plistFile) => {
    ...
    findPListFile(configuration, productName) {
        const pu = new promise_1.PromiseUtil();
        const failureString = localize(0, null);
        return pu.retryAsync(() => this.tryOneAttemptToFindPListFile(configuration, productName), // Operation to retry until successful
    ...
    tryOneAttemptToFindPListFile(configuration, productName) {
